...
class ActiveRecord implements \ArrayAccess, \IteratorAggregate {
    ...
    /**
     * @param \Webarticum\Core\Database\Table $oTable
     * @param int|null $id
    ...
    public function getRow($dataLevel = self::DATALEVEL_MERGED) {
        ...
        switch ($dataLevel) {
            ...
            case self::DATALEVEL_MERGED:
                return empty($this->loadedRow) ? $this->changesRow : $this->changesRow + $this->loadedRow;
        }
        return [];
    }
    ...
    /**
     * Sets the value for multiple fields
     *
     * @param array $updates
     */
    public function setFields($updates) {
    ...
    public function commit() {
        ...
        if (is_null($this->id)) {
            $this->id = $this->extractId($this->changesRow);
            if (is_null($this->id)) {
    ...
    /**
     * Exctracts the ID from a row
     *
     * @return mixed
     */
    public function extractId(array $row) {
        $uniqueKey = $this->oTable->getUniqueKey();
        if (is_array($uniqueKey)) {
            // TODO
        } elseif (array_key_exists($uniqueKey, $row)) {
