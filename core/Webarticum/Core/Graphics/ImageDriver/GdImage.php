...
class GdImage extends AbstractImage
 {
     ...
     * @return self
     */
    public function loadFromContent($content, $type = null) {
        $res = @imagecreatefromstring($content);
        if (!$res) {
            return $this->_error("Image can not be loaded");
        }
    ...
    public function drawPoint($left, $top) {
        ...
        } else if ($this->roundedPrimitives) {
    ...
    public function drawLine($left1, $top1, $left2, $top2) {
        ...
        if ($left1 == $left2) {
            ...
            if (!@imagefilledrectangle($res, $rectleft, $top1, $rectleft, $top2, $cres)) {
                return $this->_error("Line can not be drawn");
            }
        } elseif ($top1 == $top2) {
            $recttop = round($top1 - $this->weight / 2);
            if (!@imagefilledrectangle($res, $left1, $recttop, $left2, $recttop, $cres)) {
                return $this->_error("Line can not be drawn");
            }
        } else {
            $w = $left1 - $left2;
