...
namespace Webarticum\Core\Graphics\ImageDriver;

use Webarticum\Core\Graphics\Image;
use Webarticum\Core\Graphics\AbstractImage;
use Webarticum\Core\Graphics\Color;
use Webarticum\Core\Graphics\Font;
use Imagick;
use ImagickDraw;
use ImagickPixel;
...
class ImagickImage extends AbstractImage
 {
     ...
     * @param int $width
     * @param int $height
     * @param \Webarticum\Core\Graphics\Color $bgcolor
     * @return self
     */
    public function create($width = 100, $height = 100, $bgcolor = null) {try{
        if ($this->isOpen()) {
            $this->close();
        }
        if (is_null($bgcolor)) {
     ...
     * @return self
     */
    public function loadFromFile($file, $type = null) {try{
        if (!is_file($file)) {
    ...
    /**
     * Loads image from content
     *
     * @param string $content
     * @param string|null $type
     * @return self
     */
    public function loadFromContent($content, $type = null) {try{
        $this->_init_draw_settings();
        $this->oImagick = new Imagick();
        $this->oImagick->readImageBlob($content);
        return $this;
    }catch(\Exception $e){return $this->_error("Image can not be loaded");}}
     ...
     *
     * @param \Webarticum\Core\Graphics\Image
     */
    public function cloneFrom(Image $oImage) {try{
        $this->_clone_properties_from($oImage);
        $this->_init_draw_settings();
     ...
     * Sets the font
     *
     * @param \Webarticum\Core\Graphics\Font $font
     * @return self
     */
    public function setFont(Font $font) {try{
        $this->font = $font;
     ...
     * @return self
     */
    public function setFontSize($size) {try{
        $this->fontSize = $size;
     ...
     *
     * @param \Webarticum\Core\Graphics\Color $color
     * @return self
     */
    public function setColor(Color $color) {try{
        $this->color = $color;
     ...
     * @return self
     */
    public function setFillColor(Color $color) {try{
     ...
     * @return self
     */
    public function setAntialiasing($enabled) {try{
     ...
     * @return self
     */
    public function setWeight($weight) {try{
        $this->weight = $weight;
     ...
     * @return self
     */
    public function setRoundedPrimitives($rounded) {try{
     ...
     * @return self
     */
    public function cut($left, $top, $width, $height, $fill = false) {try{
        if ($fill) {
     ...
     * @return self
     */
    public function resize($width, $height, $mode = Image::RESIZE_NORMAL) {try{
        if ($width == 0 && $height == 0) {
            return $this;
        }
        $image = $this->oImagick;
     ...
     * @return self
     */
    public function rotate($angle = 180) {try{
     ...
     *
     * @param \Webarticum\Core\Graphics\Image $oImage
     * @param int $left
     * @param int $top
     * @param boolean $extend
     * @return self
     */
    public function pasteImage(Image $oImage, $left, $top, $extend = false) {try{
        $img = $this->oImagick;
     ...
     * @return self
     */
    public function drawText($text, $left, $top, $angle = 0, $align = Image::ALIGN_LEFT) {try{
        $aligns = [
     ...
     * @return self
     */
    public function getTextWidth($text, $fontsize = null) {try{
        $oDraw = $this->_create_drawer();
        if (!is_null($fontsize)) {
     ...
     * @return self
     */
    public function getTextUpperHeight($fontsize = null) {try{
        $oDraw = $this->_create_drawer();
        if (!is_null($fontsize)) {
            $oDraw->setFontSize($fontsize);
        }
     ...
     * @return self
     */
    public function drawPoint($left, $top) {try{
        $oDraw = $this->_create_drawer();
        $oDraw->setStrokeColor('none');
        $diff = $this->weight / 2;
        if ($this->weight == 1) {
     ...
     * @return self
     */
    public function drawLine($left1, $top1, $left2, $top2) {try{
        $oDraw = $this->_create_drawer();
     ...
     * @return self
     */
    public function drawRectangle($left, $top, $width, $height) {try{
     ...
     * @return self
     */
    public function drawRoundedRectangle($left, $top, $width, $height, $radius = 0, $verradius = null) {try{
        $oDraw = $this->_create_drawer(true);
     ...
     * @return self
     */
    public function drawCircle($left, $top, $radius) {try{
        $oDraw = $this->_create_drawer(true);
     ...
     * @return self
     */
    public function drawEllipse($left, $top, $width, $height) {try{
        $oDraw = $this->_create_drawer(true);
        $rx = $width / 2;
        $ry = $height / 2;
     ...
     * @return self
     */
    public function drawEllipseArcline($left, $top, $width, $height, $startangle, $endangle) {try{
        $oDraw = $this->_create_drawer();
        $oDraw->setFillColor('none');
     ...
     * @return self
     */
    public function drawEllipseArc($left, $top, $width, $height, $startangle, $endangle) {try{
        $oDraw = $this->_create_drawer(true);
     ...
     * @return self
     */
    public function drawBezier($left1, $top1, $left2, $top2, $left3, $top3, $left4, $top4) {try{
        $oDraw = $this->_create_drawer();
        $oDraw->setFillColor('none');
        $oDraw->bezier([
            ['x' => $left1, 'y' => $top1],
     ...
     * @return self
     */
    public function drawPolygon($points) {try{
        $oDraw = $this->_create_drawer(true);
     ...
     * @return self
     */
    public function fill($left, $top) {try{
        $oDraw = $this->_create_drawer();
     ...
     * @return self
     */
    public function effect($effect, $params = false) {try{
        $res = $this->oImagick;
        switch ($effect) {
     ...
     * @return self
     */
    public function setPixel($left, $top, $color = null) {try{
        $oDraw = $this->_create_drawer();
        if (is_null($color)) {
            $color = $this->color;
        }
     ...
     * @param int $left
     * @param int $top
     * @return \Webarticum\Core\Graphics\Color
     */
    public function getPixel($left, $top) {try{
     ...
     */
    public function flush($type = 'png', $flushHeader = false) {try{
        if ($flushHeader) {
     ...
     * @return string
     */
    public function get($type = 'png') {try{
        $this->oImagick->setImageFormat($type);
     ...
     * @return self
     */
    public function save($file = null, $type = null) {try{
